...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def auth_validate(data):
        ...
        if input_type == 'email':
            ...
            data = {
                "success": False,
                "message": "Telefon raqamingiz yoki emailingiz xato kiritildi"
            }
...
class ChangeUserInformationSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if password != confirm_password:
            data = {
                "message": "parolaringiz bir biriga mos emas"
            }
            raise serializers.ValidationError(data)
        if password:
    ...
    def validate_username(self, username):
        if len(username) < 5 or len(username) > 20:
            raise serializers.ValidationError(
                {
                    "message": "Username 5 ta belgidan kam 20 belgidan ko'p bo'lmasligi kerak"
                }
            )
        if username.isdigit():
            raise serializers.ValidationError(
                {
                    "message": "username faqat harflardan iborat bo'lishi kerak"
                }
            )
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def auth_validate(self, data):
        ...
        print("Kiritilgan userinput:", user_input)
        print("Foydalanuvchi turi:", check_user_type(user_input))
